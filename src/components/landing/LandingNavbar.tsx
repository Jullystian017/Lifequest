  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "py-3 bg-[var(--bg-main)]/80 backdrop-blur-xl border-b border-[var(--border-light)] shadow-lg"
          : "py-4 bg-transparent"
      }`}
    >
        ...
        <div className="hidden md:flex items-center gap-6">
          {["Features", "How it Works", "Leaderboard"].map((item) => (
            <Link
              ...
              className="text-xs font-semibold text-[var(--text-secondary)] hover:text-white transition-colors relative group uppercase tracking-wider"
            >
              {item}
